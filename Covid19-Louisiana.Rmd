...
ladata <- readxl::read_xlsx('./project/LA_COVID_TESTBYDAY_PARISH_PUBLICUSE.xlsx')

laoverall <- ladata %>%
  group_by(`Lab Collection Date`) %>%
  summarise(
...
laoverall <- laoverall[11:256,]
 
laoverall$positivity = laoverall$Positive / laoverall$Tests
...
df <- rbind(df, data.frame("Day"=c(-6:0), "Type" = 'Actual', value = laoverall$positivity[240:246]))
...
preds <- fore.aruma.wge(laoverall$positivity, phi = e$phi,  s=7, n.ahead = 90, lastn = T, limits = F)
...
df <- gather(df, key='Type', value, -Day)
df <- rbind(df, data.frame("Day"=c(-6:0), "Type" = 'Actual', value = laoverall$positivity[153:242]))
...
```{r PositivityRateComparison}
library(tidyverse)
library(ggthemes)
library(RColorBrewer)

us <- read_csv('./project/national-history.csv')
la <- read_csv('./project/louisiana-history.csv')

us$positivity <- us$positive/us$totalTestResults
la$positivity <- la$positive/la$totalTestResults

ggplot(us, aes(date, positive/totalTestResults)) + geom_line() 
ggplot(la, aes(date, positive/totalTestResults)) + geom_line()

us$totalTestResults
la$totalTestResults

df <- data.frame(cbind(la[c('date', 'positivity')],  us[1:260,'positivity']))
names(df) = c('date','LA','US')
df

gather(df, key = Region, value = 'Positivity Rate', -date) %>% 
...
df <- data.frame(cbind(la[c('date', 'positive')],  us[1:260,'positive']))
names(df) = c('date','LA','US')
df$US = df$US / 50

gather(df, key = Region, value = 'Positivite Tests', -date) %>% 
  ggplot(aes(date, `Positivite Tests`)) + geom_line(aes(color=Region), size=1) + labs(title='Louisiana vs US Positive Tests', x = 'Date', y='Positive Tests', subtitle = '*US numbers / 50') + theme_fivethirtyeight()
...
ASE <- mean((preds$f - us$positivity[250:256])^2)
ASE # 3.883034e-05

df <- data.frame('Predicted' = preds$f)
df$Day = row(df)
df$Actual = us$positivity[250:256]
df <- gather(df, key='Type', value, -Day)
df <- rbind(df, data.frame("Day"=c(-6:0), "Type" = 'Actual', value = us$positivity[250:256]))
df %>% ggplot(aes(Day, value, color=Type)) + geom_line() + geom_point(size=.75) + labs(title='ARIMA(1,1,1) 7-Day Forecast', x='Day', y='Positivity Rate')
...
ASE <- mean((preds$f - us$positivity[167:256])^2)
ASE # 0.000451507

df <- data.frame('Predicted' = preds$f)
df$Day = row(df)
df$Actual = us$positivity[167:256]
df <- gather(df, key='Type', value, -Day)
df <- rbind(df, data.frame("Day"=c(-89:0), "Type" = 'Actual', value = us$positivity[167:256]))
df %>% ggplot(aes(Day, value, color=Type)) + geom_line() + geom_point(size=.75) + labs(title='ARIMA(1,1,1) 90-Day Forecast', x='Day', y='Positivity Rate')
