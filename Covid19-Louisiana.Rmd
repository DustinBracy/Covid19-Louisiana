...
aic5.wge(us.d1, p=0:12, q=0:2, type='aic') # BIC says 1,1 AIC says 12,2
#aic5.wge(us.s7, p=0:12, q=0:5, type='bic')
aic5.wge(us.d1.s12, p=0:12, q=0:2, type='bic')
#plotts.sample.wge(us$positiveIncrease)

aic5.wge(us$positivity, p=0:12, q=0:2)

#e <- est.arma.wge(us.d1, p=1, q=1)

e <- est.arma.wge(us$positivity, p=11, q=1)


# Ljung Box Test does not show white noise residuals
ljung.wge(artrans.wge(us.d1, phi.tr = e$phi))
ljung.wge(artrans.wge(us.d1, phi.tr = e$phi), K = 48)
acf(e$res)
pacf(e$res)
...
for( i in 1: sims)
{
   SpecDen2 = parzen.wge(gen.aruma.wge(246,s = 12, phi = e$phi, plot ="FALSE"), plot = "FALSE")
   lines(SpecDen2$freq,SpecDen2$pzgram, lwd = 2, col = "red")
}
...
for( i in 1: sims)
{
   ACF2 = acf(gen.aruma.wge(246, s = 12, phi = e$phi, plot = "FALSE"), plot = "FALSE")
   lines(ACF2$lag ,ACF2$acf, lwd = 2, col = "red")
}
...
# Check performance
preds <- fore.aruma.wge(us$positivity, phi = e$phi, theta = e$theta, n.ahead = 7, lastn = T, limits = F)
eval_model(us$positivity,preds$f, model_name = 'AR(15) With Weekly/Monthly Trend', e$aic) #ASE .000647

preds <- fore.aruma.wge(us$positivity, phi = e$phi, theta = e$theta, n.ahead = 90, lastn = T, limits = F)
eval_model(us$positivity,preds$f, model_name = 'AR(15) With Weekly/Monthly Trend', e$aic) #ASE .000202

rolling <- rolling_ASE(us, e, s=7, horizon=7, model_name = 'AR(15) with Weekly Trend') #ASE .000277
