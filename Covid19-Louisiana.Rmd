...
eval_model <- function(response, predictions, model_name, AIC_val, ending_point=length(response)) {
  ...
  comparison_plot <- df %>% 
    ggplot(aes(Day + ending_point - num_predictions, value, color=Type)) + 
    geom_line() + 
    geom_point(size=.75) + 
    labs(title=paste(model_name, 'Performance Evaluation'),
         subtitle=paste0(num_predictions,'-Day Forecast Window'), 
         x='Day', 
         y='Positivity Rate',
...
eval_model(us$positivity,preds$f,'ARIMA(12,1,2)', e$aic) #ASE = .000156
...
# Add temp data from WeatherUnderground.com historical data for MSY
## Note Nov8 data missing from site, so imputed average of Nov7/9 was taken
xdf <- read_csv('./data/temp.csv')
xdf <- xdf[11:256,]
xdf$GatheringBinary <- ifelse(is.na(xdf$Gathering), 0,1)
```
...
eval_model(la$positivity,preds$f,'ARIMA(12,1,2) With Weekly Trend', e$aic) #ASE = .000465

preds <- fore.aruma.wge(la$positivity, phi = e$phi, theta=e$theta, d=1, s=7, n.ahead = 90, lastn = T, limits = F)
eval_model(la$positivity,preds$f,'ARIMA(12,1,2) With Weekly Trend', e$aic) #ASE = .016857
...
eval_model(la$positivity,preds$f,'ARIMA(12,1,2) With Weekly + Monthly Trends', e$aic) #ASE = .001054

########## Sig Plus Noise ########## 
preds <- fore.sigplusnoise.wge(la$positivity, max.p = 7, n.ahead = 7, limits=F)
eval_model(us$positivity,preds$f,'SigPlusNoise', 0) #ASE = .000255

preds <- fore.sigplusnoise.wge(la$positivity, max.p = 7, n.ahead = 90, limits=F)
eval_model(us$positivity,preds$f,'SigPlusNoise', 0) #ASE = .002345 (BAD)
...
eval_model(la$positivity,preds$f,'AR(8) with Weekly Trend 7-Day Forecast', e$aic) #ASE = .002049

preds <- fore.aruma.wge(la$positivity, phi = e$phi, n.ahead = 90, lastn = T, limits = F)
eval_model(la$positivity,preds$f,'AR(8) with Weekly Trend 7-Day Forecast', e$aic) #ASE = .001054
...
eval_model(us$positivity,preds$f,'ARIMA(1,1,1)', e$aic) #ASE = .000247

e$ase

#90 day
preds <- fore.aruma.wge(us$positivity, phi = e$phi, theta=e$theta, d=1, n.ahead = 90, lastn = T, limits = F)
eval_model(us$positivity,preds$f,'ARIMA(1,1,1)', e$aic)#ASE = .000395
...
#90 day
preds <- fore.aruma.wge(us$positivity, phi = e$phi, theta=e$theta, d=1, n.ahead = 90, lastn = T, limits = F)
eval_model(us$positivity,preds$f,'ARIMA(12,1,2)', e$aic) #ASE = .000472

########## Sig Plus Noise ########## 
preds <- fore.sigplusnoise.wge(us$positivity, max.p = 12, n.ahead = 7, limits=F)
eval_model(us$positivity,preds$f,'SigPlusNoise', 0) #ASE = .00019

preds <- fore.sigplusnoise.wge(us$positivity, max.p = 12, n.ahead = 90, limits=F)
eval_model(us$positivity,preds$f,'SigPlusNoise', 0) #ASE = .002618 (BAD)
...
```{r}
xdf2 <- dplyr::select(xdf, -one_of(c('Gathering','Weekday')))
xdf2 <- ts(xdf2, start = 1)

x = mlp(ts_la, hd.auto.type = 'cv', reps=100, sel.lag = T, xreg = xdf2)
plot(x)
preds <- predict(x, 90)
plot(preds)
eval_model(la$positivity,preds$mean,'NNFOR 90-Day Forecast', 0) #ASE = .00085
