...
eval_model <- function(response, predictions, pred_ul = NA, pred_ll = NA, model_name, AIC_val = 0, ending_point = length(response)) {
  ...
  #print(ggplot_build(comparison_plot)$data)
...
# Introduction
My name is Dustin Bracy, and this is a time series analysis of COVID-19 data in Louisiana and the United States spanning an 8 month period from the declaration of the pandemic on March 11, 2020 through November 11, 2020.  We will explore the difference between new daily positive cases and positivity rates, and why one may be more advantageous.  Next we'll walk through univiariate and multivariate analyses of both Louisiana and national numbers.  Finally we'll wrap up with a comparison of model performance for each use case.
...
parzen.wge(la.d1.s7, trun=100)
parzen.wge(la.s7, trun=100)

la.s7.s12 <- artrans.wge(la$positive, c(rep(0,6),1,0,0,0,0,1,0,1))
pacf(la.s7)
acf(la.s7)

parzen.wge(la.s7.s12, trun=100)
aic5.wge(la.s7.s12, type = 'bic', p=0:15,q=0:2) #12/2, 8/1, 3/2


e <- est.arma.wge(la.s7.s12,p = 12, q=2)
...
eval_model(la$positivity,preds$f, model_name = 'ARMA(12,2) With Weekly/Monthly Trend', e$aic) #ASE .000192


e <- est.arma.wge(la.s7.s12,p = 8, q=1)
plotts.wge(e$res)
ljung.wge(e$res, p=8,q=1, K=24)
ljung.wge(e$res, p=8,q=1, K=48)

preds <- fore.aruma.wge(la$positivity, phi = e$phi, theta=e$theta, s=7, n.ahead = 7, lastn = T, limits = F)
eval_model(la$positivity,preds$f, model_name = 'ARMA(8,1) With Weekly/Monthly Trend', e$aic) #ASE .000196

e <- est.arma.wge(la.s7.s12,p = 3, q=2)
plotts.wge(e$res)
ljung.wge(e$res, p=3,q=2, K=24)
ljung.wge(e$res, p=3,q=2, K=48)

preds <- fore.aruma.wge(la$positivity, phi = e$phi, theta=e$theta, s=7, n.ahead = 7, lastn = T, limits = F)
eval_model(la$positivity,preds$f, model_name = 'ARMA(3,2) With Weekly/Monthly Trend', e$aic) #ASE .000222

####

e <- est.arma.wge(la.s7.s12,p = 12, q=2)
preds <- fore.aruma.wge(la$positivity, phi = e$phi, theta=e$theta, s=7, n.ahead = 90, lastn = T, limits = F)
eval_model(la$positivity,preds$f, model_name = 'ARMA(3,2) With Weekly/Monthly Trend', e$aic) #ASE .000609

e <- est.arma.wge(la.s7.s12,p = 8, q=1)
preds <- fore.aruma.wge(la$positivity, phi = e$phi, theta=e$theta, s=7, n.ahead = 90, lastn = T, limits = F)
eval_model(la$positivity,preds$f, model_name = 'ARMA(3,2) With Weekly/Monthly Trend', e$aic) #ASE .000509

e <- est.arma.wge(la.s7.s12,p = 3, q=2)
preds <- fore.aruma.wge(la$positivity, phi = e$phi, theta=e$theta, s=7, n.ahead = 90, lastn = T, limits = F)
eval_model(la$positivity,preds$f, model_name = 'ARMA(3,2) With Weekly/Monthly Trend', e$aic) #ASE .000289
...
rolling_ASE <- function (df, fitted_model, d=0, s=0, horizon=7, training_size=30, model_name){
  ...
  for (x in 1:loop_end){
    ...
    ASE[x] = a
    comp_df$Predicted[test_start:test_stop] = preds$f
    print(a)
    plot(
      eval_model(
        data_window,
  ...
  hist(ASE)
  plot(gather(comp_df, key = Type, value = 'Positivity_Rate', -date) %>% 
         ...
         labs(
           ...
           x='Day', 
           y='Positivity Rate',
           caption=paste0('Mean ASE: ',round(mean(ASE),6))
         )
       ) 
  return(summary(ASE))
}
